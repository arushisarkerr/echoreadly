    ...
    <div className="mx-auto w-full max-w-[96rem] px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
      <section className="grid items-end gap-8 lg:grid-cols-[1.35fr_0.65fr]">
        <div>
          ...
          <p className="mt-5 max-w-md text-[0.975rem] leading-relaxed text-muted">
            Continue a listen, import something new, or open Library — all wired
            to your real files.
          </p>
        </div>
              ...
              <p className="relative mt-3 text-sm text-background/65">
                {lead.progress
                  ? [
                      ...
                      .filter(Boolean)
                      .join(" · ")
                  : `${formatFileSize(lead.item.size)} · Ready to listen`}
              </p>
            </Link>
          ) : (
            <QuietBox>
              Nothing in progress yet. Import a file to begin.
            </QuietBox>
          )}

          <Eyebrow>Recent imports</Eyebrow>
          {loading ? <QuietBox>Loading Library…</QuietBox> : null}
          {error ? <QuietBox danger>{error}</QuietBox> : null}
          {!loading && !error && recent.length === 0 ? (
            <QuietBox>Library is empty — import a file on the right.</QuietBox>
          ) : null}
      ...
      <section className="mt-14 grid gap-4 md:grid-cols-3">
        <StoryTile
          kicker="Audio"
          title="Listen"
          copy="Open a document from Library to play natural AI audio."
          href={ROUTES.library}
          cta="Open →"
        />
        <StoryTile
          kicker="Organize"
          title="Collections"
          copy="Group Library documents into reusable rooms — create, rename, and manage membership."
        ...
        <StoryTile
          kicker="Downloads"
          title="Downloads"
          copy="Download prepared MP3 narration — private to your account."
          href={ROUTES.exports}
          cta="Open →"
        />
      </section>
      ...
      <section className="mt-14 grid gap-10 border-t border-border/70 pt-10 lg:grid-cols-[0.85fr_1.15fr]">
        <div>
          ...
          <p className="mt-2 text-sm text-muted">
            Pick a narrator in Voice — listening uses your saved preference.
          </p>
